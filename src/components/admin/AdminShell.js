...
export function AdminShell({ children }) {
    ...
    return (<div className="min-h-screen bg-secondary/30">
      <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[264px] flex-col bg-ink text-ink-foreground transition-transform duration-300 lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
          {items.map((item) => {
            ...
            return (<Link key={item.to} to={item.to} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors", active ? "bg-white/12 text-white" : "text-white/70 hover:bg-white/8 hover:text-white")}>
                <Icon className="h-4 w-4"/>
                <span>{item.label}</span>
                {item.label === "Notifications" && unread?.length ? (<span className="ml-auto rounded-full bg-cognac px-2 py-0.5 text-[10px] text-white">
                    {unread.length}
                  </span>) : null}
              </Link>);
        })}
        ...
        <div className="border-t border-white/10 p-3">
          <a href="/" target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/70 transition-colors hover:bg-white/8 hover:text-white">
            <ExternalLink className="h-4 w-4"/> View store
          </a>
          <button onClick={signOut} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/70 transition-colors hover:bg-white/8 hover:text-white">
            <LogOut className="h-4 w-4"/> Sign out
          </button>
        </div>
      </aside>

      {open ? (<button aria-label="Close menu" onClick={() => setOpen(false)} className="fixed inset-0 z-40 bg-black/40 lg:hidden"/>) : null}
        ...
        <header className="sticky top-0 z-30 flex h-[76px] items-center gap-4 border-b border-border/60 bg-background/85 px-4 backdrop-blur lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-3">
            <Link to="/admin/notifications" className="relative rounded-full p-2 hover:bg-secondary">
              ...
              {unread?.length ? (<span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-cognac"/>) : null}
            </Link>
