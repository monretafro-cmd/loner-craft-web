...
export function ProductGallery({ name, items }: { name: string; items: ProductMedia[] }) {
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-5">
      {/* Thumbnails list - Desktop (Vertical Left), Mobile/Tablet (Horizontal) */}
      {count > 1 && (
        <div className={cn(
          "order-2 lg:order-1 flex w-full gap-2.5 overflow-x-auto pb-2 [scrollbar-width:none] lg:max-h-[580px] lg:w-16 lg:flex-col lg:overflow-y-auto lg:pb-0 [&::-webkit-scrollbar]:hidden shrink-0",
        )}>
          {items.map((item, i) => (
            <button
              key={item.src}
              ...
              onClick={() => setActive(i)}
              className={cn(
                "relative aspect-square shrink-0 overflow-hidden rounded-lg border-2 transition-all",
                "h-[60px] w-[60px] sm:h-[68px] sm:w-[68px] lg:h-[64px] lg:w-[64px]",
      ...
      <div className="relative min-w-0 flex-1 order-1 lg:order-2">
        <div className="relative aspect-square w-full touch-pan-y overflow-hidden rounded-2xl bg-cream p-1 md:max-h-[580px] lg:max-h-[580px]">
          {items.map((item, i) => (
            <div
              key={item.src}
