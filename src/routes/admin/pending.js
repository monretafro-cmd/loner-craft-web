...
function PendingPage() {
    ...
    if (checking && !email) {
        return (<div className="flex min-h-screen flex-col items-center justify-center bg-[#F7F3EF] p-4 text-[#241812]">
    ...
    return (<div className="flex min-h-screen items-center justify-center bg-[#F7F3EF] px-4 py-12 text-[#1C1815]">
      <div className="w-full max-w-[480px] rounded-2xl border border-[#241812]/10 bg-white p-8 text-center shadow-xl sm:p-10">
        ...
        <div className="mt-6 space-y-4">
          ...
          {(name || email) && (<div className="rounded-xl bg-[#F7F3EF] p-4 text-left border border-[#241812]/5">
        ...
        <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Button onClick={() => checkStatus()} disabled={checking} className="h-11 w-full bg-[#8A4D25] text-white hover:bg-[#8A4D25]/90">
            {checking ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <RefreshCcw className="mr-2 h-4 w-4"/>}
            Refresh Status
          </Button>
          
          <Button variant="outline" onClick={signOut} className="h-11 w-full border-[#241812]/20 text-[#241812] hover:bg-[#241812]/5">
            <LogOut className="mr-2 h-4 w-4"/>
            Sign Out
          </Button>
        </div>
        
        <a href="/" className="mt-6 inline-flex items-center text-sm font-medium text-[#8A4D25] hover:underline">
