...
export const syncAdminAccess = createServerFn({ method: "POST" })
    ...
    const ownerNeedsPromotion = isInitialOwner &&
    ...
    await supabaseAdmin.from("audit_logs").insert({
        ...
        record_type: "auth",
        record_id: userId,
        new_value: { provider, status, role },
    });
    return {
        status,
...
export const inviteAdminByEmail = createServerFn({ method: "POST" })
    ...
    const isApprovedSuperAdmin = profile?.status === "approved" && roles?.some((row) => row.role === "super_admin");
    if (!isApprovedSuperAdmin)
        throw new Error("Only an approved Super Admin can invite admins.");
...
export const logFailedLogin = createServerFn({ method: "POST" })
    ...
    await supabaseAdmin.from("audit_logs").insert({
        ...
        page: "admin/login",
        record_type: "auth",
        new_value: { provider: data.provider, reason: data.reason },
    });
    return { ok: true };
});
